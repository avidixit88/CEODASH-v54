...
# 11/559 bottleneck.
MAX_FAST_SCREEN_SPONSORS = 90
MAX_FAST_SCREEN_ITEMS_PER_SPONSOR = 4
MAX_FAST_SCREEN_SECONDS = 14.0
FAST_SCREEN_TIMEOUT_SECONDS = 2.2
MAX_PROMOTED_SCREEN_ITEMS = 30
...
def _freshness(published_at: str, text: str) -> tuple[str, float, int | None, str]:
    ...
    if year is not None:
        if year < CURRENT_YEAR:
            return "stale_historical_event", 0.05, year, f"conference/event year {year} is older than current year {CURRENT_YEAR}"
        if year == CURRENT_YEAR:
            return "upcoming_catalyst", 1.0, year, f"conference/event year {year} is current"
        return "upcoming_catalyst", 0.95, year, f"conference/event year {year} is future"

    if pub_date is None:
        return "unknown", 0.35, None, "no reliable publication date"
    age = (today - pub_date).days
    if age < 0:
        return "upcoming_catalyst", 0.95, None, "dated in the future"
    if age <= 30:
        return "recent", 1.0, None, "published within 30 days"
    if age <= 90:
        return "recent", 0.75, None, "published within 90 days"
    if age <= 180:
        return "aging", 0.45, None, "published within 180 days"
    if age <= 365:
        return "aging", 0.25, None, "published within 365 days"
    return "stale_historical_event", 0.10, None, "older than 365 days"
...
def _source_universe(
    ...
    # Rank mapped/ticker sources first, then high-relevance discovered sponsors.
    return sorted(
        by_name.values(),
        key=lambda s: (0 if s.tickers else 1, s.priority, s.sponsor.lower()),
    )
...
def _keep_executive_item(item: SponsorEvidenceItem) -> bool:
    if item.evidence_state in {"stale_historical_event", "rejected_low_lane_relevance"}:
        return False
    if item.freshness_state == "stale_historical_event":
        return False
...
def build_sponsor_evidence_summary(
    ...
    for source in universe[:MAX_FAST_SCREEN_SPONSORS]:
